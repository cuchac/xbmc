...
CIPID::CIPID(CIPFS* ipfs, unsigned int lifetimeSecs) : m_ipfs(ipfs), m_lifetimeSecs(lifetimeSecs)
{
}

CVariant CIPID::Resolve(const std::string& did)
{
  std::string method; // unused
...
CVariant CIPID::Publish(const PrivateKey& privateKey, const CVariant& content)
{
  ...
  const CCID cid = m_ipfs->PutDAG(content);
  const std::string path = "/ipfs/" + cid.ToBaseEncodedString();
